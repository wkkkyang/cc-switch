...
impl Default for MultiAppConfig {
    fn default() -> Self {
        ...
        apps.insert("qwen".to_string(), ProviderManager::default()); // 新增
...
impl MultiAppConfig {
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        // 确保 qwen 应用存在（兼容旧配置文件）
        if !config.apps.contains_key("qwen") {
            config
                .apps
                .insert("qwen".to_string(), ProviderManager::default());
            updated = true;
        }
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            // Set the test home override so our wrapper function uses the temp dir
    ...
    impl Drop for TempHome {
        fn drop(&mut self) {
            // Clear the test home override first
